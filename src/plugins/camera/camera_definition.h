...
private:
    ...
    // nothing.
    std::pair<bool, std::vector<std::shared_ptr<Option>>>
    parse_options(const tinyxml2::XMLElement *options_handle,
                  const std::string &param_name,
                  std::map<std::string, std::string> &type_map);
    std::tuple<bool, std::vector<std::shared_ptr<Option>>, Option>
    parse_range_options(const tinyxml2::XMLElement *param_handle,
                        const std::string &param_name,
                        std::map<std::string, std::string> &type_map);
    std::pair<bool, Option> find_default(const std::vector<std::shared_ptr<Option>> &options,
